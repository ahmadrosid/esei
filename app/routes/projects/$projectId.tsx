...
import { User, Task, Project } from '~/utils/models.server';

import authenticator from "~/services/auth.server";

type LoaderData = { project: Project, projectId: string | undefined, user: User };
...
export const loader: LoaderFunction = async ({
    ...
    const data: LoaderData = { project: project as unknown as Project, projectId: projectId, user };
    return json(data);
};
            ...
            <div>
                ...
                <div className='py-4 space-y-2'>
                    ...
                    {showInput && (
                        <div className=''>
                            <Form method="post">
                                <input autoFocus={true} name="name" className='text-sm bg-slate-50 w-full p-3 outline-none rounded' placeholder='Task name...' />
                    ...
                    <div className='px-2'>
                        <a onClick={() => setShowInput(true)} className='inline-flex py-2 items-center gap-2 text-slate-500 cursor-pointer'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 5v14m7-7H5" />
                            </svg>
                            <span className="text-sm pr-6 hover:text-slate-700">Add task</span>
                        </a>
                    </div>
                </div>
            </div>
